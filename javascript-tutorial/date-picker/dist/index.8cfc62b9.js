...
const datePickerGridDatesList = document.querySelector(".date-picker-grid-dates");
...
function setMonthChosen() {
    ...
    (0, _dateFns.addMonths)(currentDate, -1);
    datePickerDate.dataset.currentMonthandYear = currentDate;
}
function setupDate(currentDate) {
    const datePicker = (0, _dateFns.getUnixTime)(currentDate);
    const formattedDate = (0, _dateFns.format)(currentDate, "MMMM dd, yyyy");
    calendarButton.dataset.currentDate = datePicker;
    calendarButton.innerHTML = formattedDate;
}
function setDateIntoButton(date) {
    const formattedDate = (0, _dateFns.format)(date, "MMMM dd, yyyy");
    calendarButton.dataset.currentDate = (0, _dateFns.getUnixTime)(date);
    calendarButton.innerHTML = formattedDate;
}
function setupDayButtons(currentDate) {
    datePickerGridDatesList.innerHTML = "";
    const firstDay = (0, _dateFns.startOfWeek)((0, _dateFns.startOfMonth)(currentDate));
    const lastDay = (0, _dateFns.endOfWeek)((0, _dateFns.endOfMonth)(currentDate));
    ...
    dates.forEach((date)=>{
        ...
        button.innerHTML = (0, _dateFns.getDate)(date);
        button.classList.add("date");
        if (!(0, _dateFns.isSameMonth)(date, currentDate)) button.classList.add("date-picker-other-month-date");
        if ((0, _dateFns.isSameDay)(date, currentDate)) button.classList.add("selected");
...
function getPreviousDate() {
    //1) set current date
    const currentDate = (0, _dateFns.fromUnixTime)(datePickerDate.dataset.selectedDate);
    const date = (0, _dateFns.subMonths)(currentDate, 1);
    return date;
}
function getNextDate() {
    const currentDate = (0, _dateFns.fromUnixTime)(datePickerDate.dataset.selectedDate);
    const date = (0, _dateFns.addMonths)(currentDate, 1);
    return date;
}
function setDateOnCalendarDisplay(date) {
    ...
    datePickerDate.dataset.selectedDate = (0, _dateFns.getUnixTime)(formattedDate);
}
function setDateSelectedOnCalendar(selectedDate) {}
calendarButton.addEventListener("click", (e)=>{
    datePicker.classList.toggle("show");
    const selectedDate = (0, _dateFns.fromUnixTime)(calendarButton.dataset.currentDate);
...
},{"387b0aecfe84b867":"1vXXw"}]},["86oZd","6rimH"], "6rimH", "parcelRequire9bf3")
